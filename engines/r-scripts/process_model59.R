# process_model59.R - PROCESS Model 59: Moderated Mediation (W moderates ALL paths: a, b, AND c')
#
# Model 59: X -> M -> Y, with W moderating the X->M path (a), the M->Y path (b),
#           AND the X->Y direct path (c')
#
#   Path A model:  M  = a1*X + a2*W + a3*X*W + covariates
#   Path B model:  Y  = b1*M + b2*W + b3*M*W + c1'*X + c2'*W + c3'*X*W + covariates
#                     = b1*M + b3*M*W + c1'*X + (b2+c2')*W + c3'*X*W + covariates
#
#   Note: b2 and c2' are both linear W terms in the Y model; they are estimated
#         as a single coefficient for W in the outcome regression. The user interpretation
#         treats the moderator main effect in Y as reflecting both b2 and c2'.
#
#   Conditional indirect effect at W:  (a1 + a3*W) * (b1 + b3*W)
...
#   path_b_model           : coefficients for Y ~ M + W + M*W + X + X*W (+ covariates)
...
n_min <- length(all_col_names) + 5L
if (n < n_min) {
  stop(paste0(
    "Insufficient complete observations (n = ", n, ") for PROCESS Model 59. ",
...
xw_name <- paste0(pred_name, "_x_", mod_name)   # X*W (a-path and c'-path interaction)
mw_name <- paste0(med_name,  "_x_", mod_name)   # M*W (b-path interaction)
...
# ---- Path B model: Y ~ M + M*W + X + W + X*W + covariates ----
# Coefficients:
#   mediator           = b1  (M coef)
#   med_x_mod          = b3  (M*W coef)
#   predictor          = c1' (X coef)
#   moderator          = combined W main effect (b2 + c2' conflated)
#   pred_x_mod         = c3' (X*W coef)

b_terms <- c(med_name, mw_name, pred_name, mod_name, xw_name, cov_names)
...
path_b_coefs <- list(
  ...
  mediator        = extract_coef(smry_b, med_name),    # b1
  med_x_mod       = extract_coef(smry_b, mw_name),     # b3
  predictor       = extract_coef(smry_b, pred_name),   # c1'
  moderator       = extract_coef(smry_b, mod_name),    # b2 + c2' (combined W main effect)
  pred_x_mod      = extract_coef(smry_b, xw_name)      # c3'
)
...
b1   <- unname(coef(fit_b)[med_name])    # M coef in Y model
b3   <- unname(coef(fit_b)[mw_name])     # M*W coef in Y model
...
# ---- Bootstrap helper for conditional indirect ----

run_bootstrap_model59 <- function(w_val, seed_offset = 0L) {
  ...
  boot_fn <- function(data, indices) {
    ...
    b_r  <- build_rhs(c(local_med, local_mw, local_pred, local_mod, local_xw, local_cov))
    fb   <- tryCatch(lm(as.formula(paste(local_outcome, "~", b_r)), data = d), error = function(e) NULL)
    if (is.null(fb)) return(NA_real_)
    bb1  <- tryCatch(coef(fb)[local_med], error = function(e) NA_real_)
    bb3  <- tryCatch(coef(fb)[local_mw],  error = function(e) NA_real_)

    unname((ba1 + ba3 * local_w_val) * (bb1 + bb3 * local_w_val))
  }
...
for (i in seq_along(probe_labels)) {
  ...
  ci_info <- run_bootstrap_model59(w_val, seed_offset = as.integer(i))
...
# ---- Compute conditional direct effects (from regression vcov) ----
...
for (i in seq_along(probe_labels)) {
  ...
  # SE of conditional direct: sqrt(Var(c1') + w^2*Var(c3') + 2w*Cov(c1',c3'))
  idx_c1p <- which(names(coef_b) == pred_name)
  idx_c3p <- which(names(coef_b) == xw_name)
  de_se   <- tryCatch({
  ...
  de_p <- if (!is.na(de_t)) 2 * pt(-abs(de_t), df = n - length(coef_b)) else NA_real_
...
# Indirect effect is a quadratic function of W:
#   IE(W) = (a1 + a3*W)(b1 + b3*W) = a1*b1 + (a1*b3 + a3*b1)*W + a3*b3*W^2
# Linear component (primary IMM) = a1*b3 + a3*b1
# Quadratic component = a3*b3

imm_linear  <- a1 * b3 + a3 * b1
imm_quad    <- a3 * b3
...
sig_indirect_labels <- probe_labels[vapply(probe_labels, function(lbl) {
  isTRUE(cond_indirect_out[[lbl]]$significant)
}, logical(1L))]

interp_parts <- c(
  sprintf(
    "PROCESS Model 59 moderated mediation tested whether the indirect effect of '%s' on '%s' through '%s' was moderated by '%s' on the a-path (X->M), the b-path (M->Y), and the direct path (X->Y).",
    pred_name, outcome_name, med_name, mod_name
  ),
  sprintf(
    "N = %d complete cases. Conditional indirect effects estimated via %s at %.0f%% CI.",
    n, method, ci_pct
  ),
  if (length(sig_indirect_labels) == 0) {
    "None of the conditional indirect effects were significant."
  } else {
    sprintf(
      "Significant conditional indirect effects at %s W value(s): %s.",
      length(sig_indirect_labels), paste(sig_indirect_labels, collapse = ", ")
    )
  },
  if (!is.na(imm_sig)) {
    sprintf(
      "Index of moderated mediation (linear) = %.4f; %s CI [%.4f, %.4f]; %s.",
      imm_linear,
      paste0(ci_pct, "%"),
...
diagram <- list(
  ...
  coefficients = list(
    ...
    b1          = na_to_null(path_b_coefs$mediator$coef),
    b3          = na_to_null(path_b_coefs$med_x_mod$coef),
    c1Prime     = na_to_null(path_b_coefs$predictor$coef),
    wMainEffect = na_to_null(path_b_coefs$moderator$coef),
    c3Prime     = na_to_null(path_b_coefs$pred_x_mod$coef)
  ),
  pValues = list(
    a1          = na_to_null(path_a_coefs$predictor$p),
    a2          = na_to_null(path_a_coefs$moderator$p),
    a3          = na_to_null(path_a_coefs$interaction$p),
    b1          = na_to_null(path_b_coefs$mediator$p),
    b3          = na_to_null(path_b_coefs$med_x_mod$p),
    c1Prime     = na_to_null(path_b_coefs$predictor$p),
    wMainEffect = na_to_null(path_b_coefs$moderator$p),
    c3Prime     = na_to_null(path_b_coefs$pred_x_mod$p)
  ),
