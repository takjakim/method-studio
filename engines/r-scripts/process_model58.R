# process_model58.R - PROCESS Model 58: Moderated Mediation (W moderates a-path AND b-path)
#
# Model 58: X -> M -> Y, with W moderating BOTH the X->M path (a-path) AND the M->Y path (b-path)
#
#   Path A model:  M  = a1*X + a2*W + a3*X*W + covariates
#   Path B model:  Y  = b1*M + b2*W + b3*M*W + c'*X + covariates
#
#   Conditional indirect effect at W:  (a1 + a3*W) * (b1 + b3*W)
#   Direct effect (c') is NOT moderated in Model 58
#
...
#   path_b_model           : coefficients for Y ~ M + W + M*W + X (+ covariates)
#   conditional_indirect   : list at low/mean/high W
#   direct                 : unmoderated direct effect of X on Y
...
if (n < n_min) {
  stop(paste0(
    "Insufficient complete observations (n = ", n, ") for PROCESS Model 58. ",
...
xw_name <- paste0(pred_name, "_x_", mod_name)   # X*W (a-path interaction)
...
# ---- Path B model: Y ~ M + W + M*W + X + covariates ----
# b1 = coef of M; b2 = coef of W; b3 = coef of M*W; c' = coef of X

b_terms <- c(med_name, mod_name, mw_name, pred_name, cov_names)
...
path_b_coefs <- list(
  ...
  moderator   = extract_coef(smry_b, mod_name),    # b2
  interaction = extract_coef(smry_b, mw_name),     # b3
  predictor   = extract_coef(smry_b, pred_name)    # c'
)
r2_b     <- smry_b$r.squared
adj_r2_b <- smry_b$adj.r.squared

# Direct effect: unmoderated c'
...
run_bootstrap_model58 <- function(w_val, seed_offset = 0L) {
  ...
  boot_fn <- function(data, indices) {
    ...
    b_r  <- build_rhs(c(local_med, local_mod, local_mw, local_pred, local_cov))
...
for (i in seq_along(probe_labels)) {
  ...
  ci_info <- run_bootstrap_model58(w_val, seed_offset = as.integer(i))
...
# For Model 58 the indirect effect is a quadratic function of W:
#   IE(W) = (a1 + a3*W)(b1 + b3*W) = a1*b1 + (a1*b3 + a3*b1)*W + a3*b3*W^2
# The linear component of the IMM = a1*b3 + a3*b1 (first-order slope)
# The quadratic component = a3*b3
# Report both; bootstrap the linear term as the primary IMM index.
...
interp_parts <- c(
  sprintf(
    "PROCESS Model 58 moderated mediation tested whether the indirect effect of '%s' on '%s' through '%s' was moderated by '%s' on both the a-path (X->M) and the b-path (M->Y).",
...
diagram <- list(
  ...
  coefficients = list(
    ...
    b2          = na_to_null(path_b_coefs$moderator$coef),
    b3          = na_to_null(path_b_coefs$interaction$coef),
    cPrime      = na_to_null(path_b_coefs$predictor$coef)
  ),
  pValues = list(
    ...
    b2          = na_to_null(path_b_coefs$moderator$p),
    b3          = na_to_null(path_b_coefs$interaction$p),
    cPrime      = na_to_null(path_b_coefs$predictor$p)
  ),
